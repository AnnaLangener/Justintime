...
run_simulation <- function(features_sample,cv,n_bootstrap,testsize, seed = "12361488"){
  ...
  #  Prepare training and testing sets
    ...
    if(cv == "subject-wise" | cv == "row-wise"){
      ...
      acc_base[i] <- mean(as.numeric(as.character(class_pred_base)) == test_Y)
...
run_simulation_slidingwindow <- function(features_sample,n_bootstrap,windowsize){
  ...
  for (i in 1:n_bootstrap) {
    ...
    for(k in 1:length(trainSlices)){
      ...
      #### Baseline
      Baseline <- glmer(
        train_Y ~ 1 + (1 | subject), # Random intercept for 'subject'
    ...
    auc_value[i] <- mean(auc_value_sw,na.rm = TRUE)
    ind[[i]] <- ind_t
    auc_value_base[i] <- mean(auc_value_base_sw,na.rm = TRUE)
  } # end bootstrap

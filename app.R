    ...
    tabPanel("Upload your data",
             sidebarLayout(
               sidebarPanel(
                 ...
                 checkboxInput("shuffle", "Shuffle Outcome?", value = FALSE),
               ...
               mainPanel(
                 accordion( 
                  ...
                  accordion_panel( 
                    ...
                    h5("Baseline Model: Random-Intercept Only"),
...
server <- function(input, output, session) {
  ...
  output$cv_image <- renderUI({
    ...
    img_src <- switch(input$split_method_own,
                      "moving-window" = "figures/moving-window.png",
                      "row-wise" = "figures/Record-wise.png",
                      "subject-wise" = "figures/Subject-wise.png")
    tags$img(src = img_src, width = "500px", height = "300px", style = "border-radius: 10px; box-shadow: 2px 2px 10px grey;")
  })
  ...
  analyzed_data <- eventReactive(input$analyze_data, {
    ...
    # If shuffling is enabled, apply the shuffle function.
    if (input$shuffle) {
  ...
  output$descriptives <- renderTable({
    ...
    data.frame(
      ...
      "ICC Outcome"                 = round(model_result$icc_data, 3)
    )
  })
  ...
  output$model_results_text <- renderText({
    ...
    if (input$split_method_own == "row-wise") {
      within_person_text <- paste0(
        "Since you selected a 'row-wise' cross-validation strategy, within-person variability is an important aspect to consider. ",
      ...
      within_person_text <- paste0(
        "Since you selected a 'subject-wise' cross-validation strategy, within-person performance metrics are not relevant in this context."
      )
    } else if (input$split_method_own == "moving-window") {
      within_person_text <- paste0(
        "For within-person performance, the model achieved a mean AUC of ", mean_auc_within, 
